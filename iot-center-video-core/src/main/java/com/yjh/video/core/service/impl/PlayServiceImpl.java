...
import com.yjh.video.core.config.exception.ErrorCode;
import com.yjh.video.core.config.exception.ServiceException;
import com.yjh.video.core.config.exception.SsrcTransactionNotFoundException;
...
public class PlayServiceImpl implements IPlayService {
    ...
    public PlayResult play(MediaServerItem mediaServerItem, String deviceId, String channelId,
        ...
        if (mediaServerItem == null) {
            throw new ServiceException(ErrorCode.ERROR100.getCode(), "未找到可用的zlm");
        }
                    ...
                    if (!mediaServerItem.isRtpEnable() || device.isSsrcCheck()) {
                        ...
                        if (!mediaServerItem.isRtpEnable()) {
                            ...
                            HookSubscribeForStreamChange
                                hookSubscribe = HookSubscribeFactory.on_stream_changed("rtp", stream, true, "rtsp", mediaServerItem.getId());
    ...
    public DeferredResult<WVPResult<StreamInfo>> playBack(MediaServerItem mediaServerItem, SSRCInfo ssrcInfo,
        ...
        Device device = storager.queryVideoDevice(deviceId);
        if (device == null) {
            throw new ServiceException(ErrorCode.ERROR100.getCode(), "设备： " + deviceId + "不存在");
        }
    ...
    public DeferredResult<WVPResult<StreamInfo>> download(MediaServerItem mediaServerItem, SSRCInfo ssrcInfo, String deviceId, String channelId, String startTime, String endTime, int downloadSpeed, InviteStreamCallback infoCallBack, PlayBackCallback hookCallBack) {
        ...
        Device device = storager.queryVideoDevice(deviceId);
        if (device == null) {
            throw new ServiceException(ErrorCode.ERROR400.getCode(), "设备：" + deviceId + "不存在");
        }
