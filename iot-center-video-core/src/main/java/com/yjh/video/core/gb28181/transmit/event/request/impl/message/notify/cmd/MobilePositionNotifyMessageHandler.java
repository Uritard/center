package com.yjh.video.core.gb28181.transmit.event.request.impl.message.notify.cmd;

import com.alibaba.fastjson.JSONObject;
import com.yjh.video.core.config.UserSetting;
import com.yjh.video.core.gb28181.bean.*;
import com.yjh.video.core.gb28181.transmit.event.request.SIPRequestProcessorParent;
import com.yjh.video.core.gb28181.transmit.event.request.impl.message.IMessageHandler;
import com.yjh.video.core.gb28181.transmit.event.request.impl.message.notify.NotifyMessageHandler;
import com.yjh.video.core.service.IDeviceChannelEventService;
...
import com.yjh.video.core.utils.NumericUtil;
...
public class MobilePositionNotifyMessageHandler extends SIPRequestProcessorParent implements InitializingBean, IMessageHandler {
    ...
    @Autowired
    private IDeviceChannelEventService deviceChannelService;
            ...
            taskExecutor.execute(() -> {
                while (!taskQueue.isEmpty()) {
                    ...
                    try {
                        ...
                        mobilePosition.setChannelId(XmlUtil.getText(rootElementAfterCharset, "DeviceID"));
                        mobilePosition.setTime(XmlUtil.getText(rootElementAfterCharset, "Time"));
                        mobilePosition.setLongitude(Double.parseDouble(XmlUtil.getText(rootElementAfterCharset, "Longitude")));
                        mobilePosition.setLatitude(Double.parseDouble(XmlUtil.getText(rootElementAfterCharset, "Latitude")));
                        if (NumericUtil.isDouble(XmlUtil.getText(rootElementAfterCharset, "Speed"))) {
                            mobilePosition.setSpeed(Double.parseDouble(XmlUtil.getText(rootElementAfterCharset, "Speed")));
                        } else {
                            mobilePosition.setSpeed(0.0);
                        }
                        if (NumericUtil.isDouble(XmlUtil.getText(rootElementAfterCharset, "Direction"))) {
                            mobilePosition.setDirection(Double.parseDouble(XmlUtil.getText(rootElementAfterCharset, "Direction")));
                        } else {
                            mobilePosition.setDirection(0.0);
                        }
                        if (NumericUtil.isDouble(XmlUtil.getText(rootElementAfterCharset, "Altitude"))) {
                            mobilePosition.setAltitude(Double.parseDouble(XmlUtil.getText(rootElementAfterCharset, "Altitude")));
                        } else {
