...
import com.yjh.video.core.service.IDeviceAlarmEventService;
...
public class AlarmNotifyMessageHandler extends SIPRequestProcessorParent implements InitializingBean, IMessageHandler {
    ...
    @Autowired
    private IDeviceAlarmEventService deviceAlarmService;
        ...
        if (!taskQueueHandlerRun) {
            ...
            taskExecutor.execute(() -> {
                ...
                while (!taskQueue.isEmpty()) {
                    ...
                    deviceAlarm.setAlarmPriority(XmlUtil.getText(sipMsgInfo.getRootElement(), "AlarmPriority"));
                    deviceAlarm.setAlarmMethod(XmlUtil.getText(sipMsgInfo.getRootElement(), "AlarmMethod"));
                    String alarmTime = XmlUtil.getText(sipMsgInfo.getRootElement(), "AlarmTime");
                    if (alarmTime == null) {
                        continue;
                    }
                    deviceAlarm.setAlarmTime(DateUtil.ISO8601Toyyyy_MM_dd_HH_mm_ss(alarmTime));
                    String alarmDescription = XmlUtil.getText(sipMsgInfo.getRootElement(), "AlarmDescription");
                    if (alarmDescription == null) {
                        deviceAlarm.setAlarmDescription("");
                    } else {
                        deviceAlarm.setAlarmDescription(alarmDescription);
                    }
                    String longitude = XmlUtil.getText(sipMsgInfo.getRootElement(), "Longitude");
                    if (longitude != null && NumericUtil.isDouble(longitude)) {
                        deviceAlarm.setLongitude(Double.parseDouble(longitude));
                    } else {
                        deviceAlarm.setLongitude(0.00);
                    }
                    String latitude = XmlUtil.getText(sipMsgInfo.getRootElement(), "Latitude");
                    ...
                    if (!ObjectUtils.isEmpty(deviceAlarm.getDeviceId())) {
                        if (deviceAlarm.getAlarmMethod().contains(DeviceAlarmMethod.Video.getVal() + "")) {
                            deviceAlarm.setAlarmType(XmlUtil.getText(sipMsgInfo.getRootElement().element("Info"), "AlarmType"));
                        }
                    }
