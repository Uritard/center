   ...
   public Result unknownException(Exception ex) {
       ...
       if(ex instanceof BusinessException){
           ...
           if(businessException.getCode()!=null){
               return new Result(businessException.getCode(), "业务处理失败");
           }else{
               return new Result(HttpStatus.INTERNAL_SERVER_ERROR.value(), "系统异常");
           }
       }
       if(ex instanceof MethodArgumentTypeMismatchException){
           return new Result(HttpStatus.INTERNAL_SERVER_ERROR.value(),"参数类型错误，请检查参数");
