...
public class CrcCheck {
    ...
    public static void main(String[] args) {
        byte[] data = {
                ...
                0,    /*加密标志*/
                2,    /*类型*/
                0x55, 0x03, 0x14, 0x12, 0x78, 0x23, 0x05, /*桩编号*/
                0,    /*登录结果*/
        };

        byte[] crc = calcCrc16(data, 0, data.length);
        System.out.printf("0x%02X%02X", crc[0], crc[1]);
    }
     ...
     * 查表法计算CRC16校验
     */
    public static byte[] calcCrc16(byte[] buf, int start, int len) {
        int high = 0x00ff;
        int low = 0x00ff;
        int index;
        for (int i = start, end = start + len; i < end; ++i) {
            index = (low ^ buf[i]) & 0x00ff;
            low = high ^ CRC_16_H[index];
            high = CRC_16_L[index];
        }
...
//        return new Crc16((byte) (high & 0x00ff), (byte) (low & 0x00ff));
//        return String.format("%04X", crc);
    }

    static final byte[] CRC_16_H = {
