...
public class EnumCache {
    ...
    public static <E extends Enum> E find(Class<E> clazz, Object obj, Map<Class<? extends Enum>, Map<Object, Enum>> cache, E defaultEnum) {
        ...
        if (map == null) {
            ...
            if (cache == CACHE_BY_NAME) {
                msg = String.format("枚举%s还没有注册到枚举缓存中,请在%s.static代码中加入如下代码: EnumCache.registerByName(%s.class, %s.values());",
                        clazz.getSimpleName(), clazz.getSimpleName(), clazz.getSimpleName(), clazz.getSimpleName());
            }

            if (cache == CACHE_BY_VALUE) {
                msg = String.format("枚举%s还没有注册到枚举缓存中,请在%s.static代码中加入如下代码: EnumCache.registerByValue(%s.class, %s.values(), %s :: getXxx);",
    ...
    private static <E extends Enum> void executeEnumStatic(Class<E> clazz) {
        if (!LOADED.containsKey(clazz)) {
            synchronized (clazz) {
                if (!LOADED.containsKey(clazz)) {
                    try {
                        // 为了让枚举类的static块运行,static块没有执行完是会阻塞在这里的
