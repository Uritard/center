...
import com.yjh.platform.module.patrol.entity.interlanalysis.AnalyseResult;
...
/**
    ...
    public NewDataDealThread(String body, int remotePort, RedisTemplate redisTemplate, AnalyseDataOperateService analyseDataOperateService,
    ...
    public void run() {
        ...
        if ("2".equals(jsonObject.getString("msgType"))) {
            ...
            while (iterator.hasNext()) {
                ...
                response.setRequestId("selfAlgorithm" + "#" + taskId);
                List<AnalyseResult> resultsList = new ArrayList<>();
                AnalyseResult analyseResult = new AnalyseResult();
                analyseResult.setObjectId(instanceId);
                List<AnalyseResultItem> results = new ArrayList<>();
                AnalyseResultItem analyseResultItem = new AnalyseResultItem();
                analyseResultItem.setCode("2000");
                analyseResultItem.setConf(Objects.equals("13668", remotePort) ? 0.0f : 666f);
                String desc = "";
                String type = " ";
                String value = "";
                float conf = 0.0f;
                ...
                analyseResultItem.setType(type);
                analyseResultItem.setDesc(desc);
                analyseResultItem.setConf(conf);
                analyseResultItem.setValue(value);

                analyseResultItem.setPos(Collections.emptyList());
                results.add(analyseResultItem);
                analyseResult.setResults(results);
                resultsList.add(analyseResult);
