...
//    private RedisAndYxsjUtil redisAndYxsjUtil;
    ...
    public int insert(SysUser sysUser, HttpServletRequest request) throws IOException {
        ...
        logsRecord.LoginLogsSend(request, "2", "新增用户", userName + "用户新增了" + sysUser.getUserName() + "用户", userName, String.valueOf(userIds), 1);
        return total;
    }
    ...
    public int deleteByPrimaryId(Long userId, HttpServletRequest request) {
        ...
        logsRecord.LoginLogsSend(request, "4", "删除系统用户数据", userName + "用户删除了" + userNames + "用户", userName, String.valueOf(userId), 1);
        ...
        if (!number.equals(replayAvoid)) {
            ...
            if (userMap.size() > 0 && !Objects.equals(null, userMap.get("userName")) && !Objects.equals(null, userMap.get("pCode"))) {
                ...
                if (Objects.equals(null, sysUserLogin)) {
                    logsRecord.LoginLogsSend(request, "6", "登录", "用户名或密码错误登录失败", userName, "", 2);
                ...
                if (!Objects.equals(null, sysUserLogin) && Demo.decryptDB(sysUserBackUp.getPassword()).equals(password) && userName.equals(sysUserLogin.getUserName())) {
                    ...
                    if (sysUserLogin.getState().intValue() == 2) {
                        ...
                        if (lockTime < Integer.valueOf(lockTimes.get("content")) * 60) { //如果锁定时间小于1200S
                         ...
                         //   mapResult.put("errorCount", "账户已被锁定！");
                            ...
                            mapResult.put("info", "账户也被锁定，剩余锁定时间"+surplusTime+"分钟");
                            return mapResult;
                        } else {
                    ...
                    if (sysUserLogin.getState().intValue() == 0) {
                        logsRecord.LoginLogsSend(request, "6", "登录", "用户名或者密码错误", userName, String.valueOf(sysUserLogin.getUserId()), 2);
                        mapResult.put("code", ResultCodeEnum.CODE10101.getCode());
                        mapResult.put("info", ResultCodeEnum.CODE10101.getName());
                        return mapResult;
                    }
                    if (sysUserLogin.getState().intValue() == 1) {
                        if (yxTime >= sysUserLogin.getInvalidTime() - 1 && yxTime < sysUserLogin.getInvalidTime()) {
                            mapResult.put("pwdExpirationTip", "密码还有一天即将到期，请及时更换密码！");
                        } else if (yxTime >= sysUserLogin.getInvalidTime()) {
                            logsRecord.LoginLogsSend(request, "6", "登录", userName + "账户密码超期登录失败，请联系管理员处理！", userName, String.valueOf(sysUserLogin.getUserId()), 3);
                    ...
                    if (sysUser.getState().intValue() == 2) {
                        ...
                        Long lockTime = DateTimeUtil.sencondsBetween(calendarOne, calendar);
                        if (lockTime < Integer.valueOf(lockTimes.get("content")) * 60) { //如果锁定时间小于1200S
                            logsRecord.LoginLogsSend(request, "6", "登录", userName + "账户已被锁定！", userName, String.valueOf(sysUserLogin.getUserId()), 3);
                          ...
                          //  mapResult.put("errorCount", "账户也被锁定，剩余锁定时间"+surplusTime+"分钟");
                            ...
                            logsRecord.LoginLogsSend(request, "6", "登录", userName + "账户已被锁定！", userName, String.valueOf(sysUserLogin.getUserId()), 3);
                            logsRecord.LoginLogsSend(request, "6", "登录", "用户账户有泄漏风险", userName, String.valueOf(sysUserLogin.getUserId()), 2);
                            mapResult.put("info", "账户也被锁定，剩余锁定时间"+lockTimeOne+"分钟");
                            mapResult.put("code", ResultCodeEnum.CODE10102.getCode());
                            return mapResult;
                        }
                    } else {
