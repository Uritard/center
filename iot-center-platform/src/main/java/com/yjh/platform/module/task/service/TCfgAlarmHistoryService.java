...
public class TCfgAlarmHistoryService{
    ...
    @Logs(title = "分页查询", code = "TCfgAlarmHistory",content = "根据web传入的参数查询")
    @Transactional(rollbackFor = Exception.class)
    ...
    @Logs(title = "批量插入", code = "TCfgAlarmHistory",content = "根据web传入的参数批量插入")
    @Transactional(rollbackFor = Exception.class)
    public int batchInsert(List<TCfgAlarmHistory> list) {
        return this.tCfgAlarmHistoryDao.batchInsert(list);
    }

    @Logs(title = "查询联动详细结果", code = "TCfgAlarmHistory",content = "查询联动任务的详细结果")
    @Transactional(rollbackFor = Exception.class)
