...
public class TCruiseTaskResultController {
    ...
    @Logs(title = "删除任务点状态数据",content = "根据用户传递的参数删除任务点状态数据",logType = 4)
    ...
    @Logs(title = "查询任务点状态数据",content = "根据用户传递的参数查询任务点状态信息",logType = 1)
    ...
    @Logs(title = "查询任务点状态数据",content = "根据用户传递的参数分页查询任务点状态信息",logType = 1)
    public Result selectByPage(@RequestBody TCruiseTaskResult tCruiseTaskResult
                               ) {
        Result result = new Result();
        Map<String, Object> resultMap = new HashMap<>();
        try {
            Page page = PageHelper.startPage(tCruiseTaskResult.getPageNum()!=null?tCruiseTaskResult.getPageNum():1, tCruiseTaskResult.getPageSize()!=null?tCruiseTaskResult.getPageSize():0,true,null,true);
...
//    @Logs(title = "获取当前任务下的巡检点的执行信息",content = "根据用户传递的参数获取当前任务下巡检点的执行信息",logType = 1,authority = "1235")
    ...
    public Result cruiseCameraStop(@RequestParam String taskId){
        Result result=new Result();
        try {
            result.setData(tCruiseTaskResultService.cruiseCameraStop(taskId));
        } catch (Exception e) {
            result.setCode(ResultCodeEnum.UPDATEERROR.getCode(), ResultCodeEnum.UPDATEERROR.getName());
            log.error("失败描述：", e);
        }
    ...
    @Logs(title = "获取当前执行任务的实时告警信息",content = "根据用户传递的参数获取当前执行任务的实时告警信息",logType = 1,authority = "1234,1235")
    ...
    @Logs(title = "获取巡检任务进度",content = "根据用户传递的参数获取巡视任务进度",logType = 1,authority = "1234,1235")
...
//            CruiseResultCounter cruiseResultCounter = tCruiseTaskResultService.selectCruiseStatusCount(taskId);
    ...
    @Logs(title = "获取当前任务下的摄像头/机器人信息以及其工作的巡检点结果状态",content = "根据用户传递的参数获取当前任务下的摄像头、机器人信息",logType = 1)
    ...
    @Logs(title = "巡视结果图片比对",content = "根据用户传递的参数对巡视结果图片对比",logType = 5)
    ...
    @Logs(title = "读取缓存中的任务下巡视点绑定的摄像头信息",content = "读取缓存中任务下的巡视点绑定摄像头信息",logType = 1)
