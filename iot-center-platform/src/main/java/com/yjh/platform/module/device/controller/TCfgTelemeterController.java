...
public class TCfgTelemeterController {
    ...
    @Logs(title = "删除遥测数据",content = "根据用户传递的参数删除遥测数据",logType = 4)
    ...
    @Logs(title = "查询遥测数据",content = "根据用户传递的参数查询遥测信息",logType = 1)
    ...
    @Logs(title = "批量插入遥测数据",content = "根据用户传递的参数批量插入遥测信息",logType = 2)
    public Result batchAdd(@Validated @RequestBody List<TCfgTelemeter> list) {
        Result result = new Result();
        try {
