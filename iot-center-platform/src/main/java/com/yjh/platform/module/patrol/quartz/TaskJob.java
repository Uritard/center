...
public class TaskJob extends QuartzJobBean {
    ...
    public void executeInternal(JobExecutionContext context) {
        ...
        UPatrolTask task = uPatrolTaskDao.selectThisTaskByTaskCode(Optional.ofNullable(ancestralTask).map(UPatrolTask::getTaskCode).orElse("notfound"), date);
        ...
        if (task.getExecuteType() == CruiseConstant.TaskTypeEnum.CYCLE.getType()) {
            if (CollectionUtils.isNotEmpty(tCruiseTaskDelList)) {
                ...
                uPatrolTaskService.deleteJob(taskId);
                return;
        ...
        boolean canRunning = pauseFormerLowerTask(task);
    ...
    private void droneTaskStart(UPatrolTask task) {
        ...
        robotCodes.forEach(robotCode -> {
            try {
                ...
                ScheduledMapConfig.schedule(6, robotTaskStatesMap, t -> uPatrolTaskService.robotTaskStates(t));
    ...
    private boolean pauseFormerLowerTask(UPatrolTask task) {
        try {
            if (task.getTaskLevel() != null) {
                ...
                boolean pauseFlag = uPatrolTaskService.highTaskPause(taskId, highTaskList);
                // 有高优先级任务，当前任务暂停
                if (pauseFlag) {
                    return false;
                }
                ...
                List<String> interList = uPatrolTaskService.cruiseDeviceIntersect(taskId, lowTaskList);
