...
public class TCfgTelesignalController {
    ...
    @Logs(title = "删除遥信数据",content = "根据用户传递的参数删除遥信数据",logType = 4)
    ...
    @Logs(title = "更新遥信数据",content = "根据用户传递的参数修改遥信数据",logType = 3)
    public Result update(@Validated @RequestBody TCfgTelesignal tCfgTelesignal) {
        Result result = new Result();
        try {
    ...
    @Logs(title = "查询遥信数据",content = "根据用户传递的参数查询遥信数据",logType = 1)
