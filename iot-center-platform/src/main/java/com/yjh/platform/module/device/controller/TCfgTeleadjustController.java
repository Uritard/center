...
public class TCfgTeleadjustController {
    ...
    public Result insert(@RequestBody TCfgTeleadjust tCfgTeleadjust) {
    ...
    public Result select(@RequestParam(value = "deviceId", required = false) String deviceId,
        ...
        Result result = new Result();
        try {
            List<TCfgTeleadjust> list = tCfgTeleadjustService.select(deviceId, meteId, meteName, upEffect, lowEffect, metePrecision, unit, meteIndex, meteCid, adjustKind, lastValue, lastTime, meteCode, deviceType, description, stander, controlenable);
            result.setData(list);
        } catch (Exception e) {
            result.setCode(ResultCodeEnum.UPDATEERROR.getCode(), ResultCodeEnum.UPDATEERROR.getName());
            log.error("遥调量表操作失败描述：", e);
    ...
    public Result batchInsert(@RequestBody List<TCfgTeleadjust> list) {
        Result result = new Result();
        try {
