...
INSERT INTO `system_config` (`config_type`, `config_name`, `config_key`, `config_value`, `config_remark`, `remark`, `rules`) VALUES ('algorithmSystem', '分析主机配置', 'algorithmSystemFtpsResolveLocal', '0', '传输时使用连接地址', 'FTPS隐式传输时，使用登录IP作为数据传输的地址，不使用服务端返回的IP，避免外网可以访问而内网不可以', '{\"rule\":\"^(true|false)$\",\"msg\":\"只能填 true 或 false\"}');
