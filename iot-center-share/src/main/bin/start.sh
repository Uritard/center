...
echo 'stop process:'$stopjar'  '$process_exists
if [ -n "${process_exists}" ];then
    kill -9 ${process_exists}
fi

JAVA_OPTS="-server -Xms768m -Xmx768m -XX:MetaspaceSize=128m -XX:+UseConcMarkSweepGC -XX:CMSInitiatingOccupancyFraction=75"
