...
import com.yjh.accessvideo.hik.CompressionAudio;
import com.yjh.accessvideo.hik.HCNetSDK;
import com.yjh.accessvideo.hik.HikDeviceInfo;
...
public class VoiceComService {
    ...
    public VoiceComService(CameraConDao cameraConDao){
    ...
    public int startVoiceCom(Long cameraId, Integer dwVoiceChan){
        ...
        HikUtilsApp hikUtilsApp = new HikUtilsApp();
        int startVoiceCom = hikUtilsApp.startVoiceCom(lUserId, dwVoiceChan);

        return startVoiceCom;
    }
    ...
    public boolean stopVoiceCom(Long cameraId){
        ...
        log.info("hikDeviceVoiceComHandleMaps:{}", Constant.hikDeviceVoiceComHandleMaps);
    ...
    public int startVoiceTransTest(Long cameraId, int dwVoiceChan, long timeItem){
        log.info("hikDeviceUserIdMaps:{}", Constant.hikDeviceUserIdMaps);
        Integer lUserId = Constant.hikDeviceUserIdMaps.get(String.valueOf(cameraId));
        HikUtilsApp hikUtilsApp = new HikUtilsApp();
        ...
        // 保存回调函数的原始音频数据
        String filePathName = property + "/AudioFile/originAudio-" + format + ".g7";
        ...
        if (2 == Constant.encodeFormat){
            ...
            File filePcm = new File(property + "/AudioFile/decodeData-" + format + ".pcm");
            if (!filePcm.exists()) {
                try {
        ...
        if (cbVoiceDataCallBack == null) {
            cbVoiceDataCallBack = new CbVoiceDataCallBack();
        }
        int startVoiceTrans = hikUtilsApp.startVoiceTrans(lUserId, dwVoiceChan, cbVoiceDataCallBack, null);
    ...
    public void voiceComSendDataTest(int lVoiceTranHandle, String fileName){
        String format = new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());

        String filePathNameTemp = property + "/AudioFile/";
        ...
        encodeInfo.write();
        // 初始化G711音频编码
        Pointer encoder = hCNetSDK.NET_DVR_InitG711Encoder(encodeInfo);
        ...
        while ((dataLength - iEncodeSize) > DATA_SIZE || ((dataLength - iEncodeSize) > 0 && (dataLength - iEncodeSize) <= DATA_SIZE)) {
            ...
            strutAudioParam.out_frame_size = 160;
            strutAudioParam.g711_type = 1;
            strutAudioParam.write();

            // G711音频编码
            if (!hCNetSDK.NET_DVR_EncodeG711Frame(encoder, strutAudioParam)) {
                log.error("NET_DVR_EncodeG711Frame failed, error code:{}", hCNetSDK.NET_DVR_GetLastError());
            ...
            for (int i = 0, size = strutAudioParam.out_frame_size / 160; i < size; i++) {
                ...
                if (!hCNetSDK.NET_DVR_VoiceComSendData(lVoiceTranHandle, ptrG711Send.byValue, 160)) {
                    log.error("NET_DVR_VoiceComSendData failed, error code:{}", hCNetSDK.NET_DVR_GetLastError());
                    // 数据发送结束,关闭编码库资源
                    hCNetSDK.NET_DVR_ReleaseG711Encoder(encoder);
                    return;
                }
    ...
    public void voiceComSendDataTest2(int lVoiceTranHandle, String fileName){
        String filePathNameTemp = property + "/AudioFile/";

        File file = new File(filePathNameTemp + fileName);
        try (FileInputStream inputStream = new FileInputStream(file)){
            ...
            if (pInBuff != Memory.NULL) {
                ...
                while (currFileLen < file.length()) {
                    ...
                    param.in_buf = pIB;
                    param.out_buf = ptrG711Data.getPointer();
                    param.out_frame_size = 160;
                    param.g711_type = 1;

                    // G711音频编码
                    if (!hCNetSDK.NET_DVR_EncodeG711Frame(encoder, param)) {
                        log.error("NET_DVR_EncodeG711Frame failed, error code:{}", hCNetSDK.NET_DVR_GetLastError());
                        hCNetSDK.NET_DVR_ReleaseG711Encoder(encoder);
                        return;
                    }

                    // 转发语音G711数据,每次发送160字节,发送间隔时间20ms
                    if (!hCNetSDK.NET_DVR_VoiceComSendData(lVoiceTranHandle, ptrG711Data.byValue, 160)) {
                        log.error("NET_DVR_VoiceComSendData failed, error code:{}", hCNetSDK.NET_DVR_GetLastError());
                        // 数据发送结束,关闭编码库资源
                        hCNetSDK.NET_DVR_ReleaseG711Encoder(encoder);
                        return;
    ...
    public boolean stopVoiceTransTest(Long cameraId){
        ...
        if (Constant.pDecHandle != null) {
            hCNetSDK.NET_DVR_ReleaseG711Decoder(Constant.pDecHandle);
        }
