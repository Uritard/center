...
@Slf4j
@Service
public class DroneInspectionResultHandler implements DroneMessageHandlerStrategy, InitializingBean {
    ...
    public void handler(ChannelHandlerContext ctx, RobotServerHandler droneServerHandler, XMLBaseModel xmlBaseModel, long sendSessionId, long receiveSessionId) throws Exception {
        ...
        droneService.uploadFile(ftpFilePath, ftpFilePath);

        // 结果文件处理及判断结果是否告警
        resultFileHandler(xmlBaseModel, ftpFilePath, cruiseResultMap);

        log.info("无人机巡视结果数据是：{}", cruiseResultMap);
        // Start CruiseResultDealThread
        InspectionResultThread cruiseResultDealThread = new InspectionResultThread(cruiseResultMap, redisTemplate, websocketUrl,false);
        TaskExecutePool.getInstance().execute(cruiseResultDealThread);

        String cruiseResultXmlString = PlatformXMLUtil.generateXml2(RobotServerHandler.sendMessageForCommandThree(true, droneCode), PlatformXMLUtil.DRONEROOTNAME);
        ...
        RobotServerHandler.send(cruiseResultProtocol, droneCode);
        log.info("巡视主机给无人机{}响应了", droneCode);

        resultUpToStation(xmlBaseModel, droneCode);
    }
     ...
     * @param ftpFilePath     无人机上报结果文件路径
     * @param cruiseResultMap 无人机巡视结果临时Map
     * @return void
     */
    private void resultFileHandler(XMLBaseModel xmlBaseModel, String ftpFilePath, Map<String, String> cruiseResultMap) {
        ...
        isAlarmMap.put("droneCode", xmlBaseModel.getSendCode());
     ...
     * @return void
     */
    public void resultUpToStation(XMLBaseModel xmlBaseModel, String droneCode) {
        Map<String, String> redisInfoMap = redisTemplate.opsForHash().entries("Drone_SPAndIN_Info:" + droneCode + ":" + xmlBaseModel.getCode());
...
//          xmlBaseModel.getItems().get(0).put("taskPatrolledId",mapForGet.get("taskId")+"_"+simpleDateFormat2.format(mapForGet.get("cruiseTime")));
        xmlBaseModel.getItems().get(0).remove("drone_code");
...
//        Constant.otherServer(cruiseResult,Constant.TCP_URL);
    ...
    public void afterPropertiesSet() throws Exception {
        DroneMessageHandlerStrategyFactory.register(DroneHandlerEnum.INSPECTION_RESULT.getCode(), this);
    }
}
