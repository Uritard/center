...
public class RobotWarnHandler implements MessageHandlerStrategy, InitializingBean {
    ...
    public void handler(ChannelHandlerContext ctx, RobotServerHandler robotServerHandler, XMLBaseModel xmlBaseModel, long sendSessionId, long receiveSessionId) throws Exception {
        ...
        if (Constant.robotRegisterFlag.getOrDefault(robotCode, false)) {
            ...
            robotAlarmMap.put("patrolDeviceName", xmlBaseModel.getItems().get(0).get("patroldevice_name").toString());
            robotAlarmMap.put("patrolDeviceCode", xmlBaseModel.getItems().get(0).get("patroldevice_code").toString());
