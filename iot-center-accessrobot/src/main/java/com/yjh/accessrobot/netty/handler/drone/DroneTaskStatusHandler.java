...
@Service
@Slf4j
public class DroneTaskStatusHandler implements DroneMessageHandlerStrategy, InitializingBean {
    ...
    public void handler(ChannelHandlerContext ctx, RobotServerHandler droneServerHandler, XMLBaseModel xmlBaseModel, long sendSessionId, long receiveSessionId) throws Exception {
        ...
        // Deal with drone task status data
        ...
        // 判断任务是否属于无人机本体任务
        Long droneId = droneService.selectIsDroneTask(taskId);
        if (Objects.nonNull(droneId)) {
            ...
            StandTaskDealThread standTaskDealThread = new StandTaskDealThread(redisTemplate, taskId, droneCode);
        ...
        Map<String, Object> listMap = redisTemplate.opsForHash().entries("DroneTaskStatus:" + droneCode + ":" + taskId);
        taskStatusMap.put("instanceList", listMap.get("instanceIdList"));
        log.info("taskStatusMap==" + taskStatusMap);
        redisTemplate.opsForHash().putAll("DroneTaskStatus:" + droneCode + ":" + taskId, taskStatusMap);

        String taskStatusXmlString = PlatformXMLUtil.generateXml2(RobotServerHandler.sendMessageForCommandThree(true, droneCode), PlatformXMLUtil.DRONEROOTNAME);
        byte[] taskStatusProtocol = PlatformPacketUtil.createPacket(Constant.sendSessionId, sendSessionId, false, taskStatusXmlString);
        RobotServerHandler.send(taskStatusProtocol, droneCode);
        ...
        if ("1".equals(taskState) || "4".equals(taskState)) {
            ...
            StaticContextAccessor.getBean(DroneService.class).updateTCruiseTask(taskMap);
        }

        // 任务根本没做或者没有完成,但是上报任务状态信息进度为100%(E无人机出现过,里面有很多重复代码,未优化)
        if (Objects.equals("1", taskState)) {
            ...
            log.info("无人机任务为{}返回结果个数===={}", taskId, resultList.size());

            // 统计巡视主机下发给无人机的巡检点大小
            Map<String, String> redisInfoMap2 = redisTemplate.opsForHash().entries("DroneTaskStatus:" + droneCode + ":" + taskId);
            ...
            log.info("巡视主机下发给无人机任务为{}的巡检点个数===={}", taskId, allInstanceIdList.size());
     ...
     *
     * @param taskId            任务id
     * @param allInstanceIdList 巡视主机下发给无人机的巡检点
     * @param taskName          任务名称
     * @return void
     */
    private void neverDone(String taskId, List<String> allInstanceIdList, String taskName) throws Exception {
        ...
        TCruiseResult tCruiseResult = droneService.selectTaskResultId(taskId);
        ...
        List<Long> isFinishedInstanceList = StaticContextAccessor.getBean(DroneService.class).selectInstanceForTaskGoOn(taskId);
        ...
        if (CollectionUtils.isNotEmpty(tctrdList)) {
            res1 = StaticContextAccessor.getBean(DroneService.class).batchInsertCruiseTaskResultDetail(tctrdList);
        }
        if (CollectionUtils.isNotEmpty(tcdrList)) {
            res2 = StaticContextAccessor.getBean(DroneService.class).batchInsertCruiseDataResult(tcdrList);
        ...
        /*
         * 判断缓存中的异常点，如果无人机任务是最后执行，则更新缓存并更新表
         */
        if (abnormal + normal == totalNum) {
            log.info("无人机巡检点是最后一个点");
            Thread.sleep(15000);
            ...
            StaticContextAccessor.getBean(DroneService.class).insertTCruiseTaskResult(tCruiseTaskResult);
            ...
            StaticContextAccessor.getBean(DroneService.class).updateTCruiseResult(tCruiseResult);
            ...
            for (TCruiseTaskResultDetail tctrd : tctrdList) {
                StaticContextAccessor.getBean(DroneService.class).updateIsWarn(taskId, tctrd.getInstanceId(), tctrd.getCruiseResultId());
            }

        } else {
            log.info("无人机巡检点不是最后一个点");
            Integer taskWait = totalNum - normal - abnormal;
            log.info("taskWait的值是==" + taskWait);
            tCruiseResult.setTaskWait(taskWait);
    ...
    public void afterPropertiesSet() throws Exception {
        DroneMessageHandlerStrategyFactory.register(DroneHandlerEnum.DRONE_TASK_STATUS.getCode(), this);
    }
}
