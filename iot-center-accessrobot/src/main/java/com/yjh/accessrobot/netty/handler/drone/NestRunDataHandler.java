...
@Service
@Slf4j
public class NestRunDataHandler implements DroneMessageHandlerStrategy, InitializingBean {
    ...
    public void handler(ChannelHandlerContext ctx, RobotServerHandler droneServerHandler, XMLBaseModel xmlBaseModel, long sendSessionId, long receiveSessionId) throws Exception {
        ...
        if (Constant.robotRegisterFlag.getOrDefault(droneCode, false)) {
            ...
            xmlBaseModel.getItems().forEach(res -> {
                ...
                droneOperationMap.put("nestName", res.get("nest_name").toString());
                droneOperationMap.put("nestCode", res.get("nest_code").toString());
                droneOperationMap.put("moduleNo", res.get("module_no").toString());
