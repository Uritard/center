...
@Slf4j
@Service
public class DroneOperationResultHandler implements DroneMessageHandlerStrategy, InitializingBean {
    ...
    public void handler(ChannelHandlerContext ctx, RobotServerHandler droneServerHandler, XMLBaseModel xmlBaseModel, long sendSessionId, long receiveSessionId) throws Exception {
        ...
        droneService.uploadFile(confirmFilePath, confirmFilePath);

        //操作结果文件处理
        operationResultFileHandler(xmlBaseModel, ftpFilePath, confirmFilePath, operationResultMap);

        log.info("无人机操作结果数据是：{}", operationResultMap);
        ...
        String cruiseResultXmlString = PlatformXMLUtil.generateXml(RobotServerHandler.sendMessageForCommandThree(true, droneCode));
    ...
    public void afterPropertiesSet() throws Exception {
        DroneMessageHandlerStrategyFactory.register(DroneHandlerEnum.OPERATION_RESULT.getCode(), this);
    }
}
