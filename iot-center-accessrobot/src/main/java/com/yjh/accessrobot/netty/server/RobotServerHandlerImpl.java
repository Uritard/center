...
public class RobotServerHandlerImpl extends ChannelInboundHandlerAdapter implements RobotServerHandler {
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
        ...
        if ("java.io.IOException: 远程主机强迫关闭了一个现有的连接。"
                .equals(cause.toString()) || "java.io.IOException: Connection reset by peer".equals(cause.toString())) {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        String allPacket = channelPacket.getOrDefault(channelId.toString(), "").toString() + str.toString()
                .replace(" ", "");
    ...
    public void openPackage(ChannelHandlerContext ctx, String socketMessageHex, int headNum) {
        ...
        if (socketMessageHex.startsWith(TAG) && headNum >= counts) {
            ...
            if (socketMessageHex.length() - limitNum == 1) {
                ...
                channelPacket.put(channelId.toString(),
                        channelPacket.get(channelId.toString()).toString().replace(socketMessageHex, ""));
