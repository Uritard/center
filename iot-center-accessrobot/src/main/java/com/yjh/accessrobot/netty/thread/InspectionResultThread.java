...
//        List<Long> instanceIdList = new ArrayList<>();
        ...
        tCruiseTaskResultMap.put("instanceName", robotInfoKeyMap.get("inspectionName"));
        tCruiseTaskResultMap.put("deviceName", robotInfoKeyMap.get("deviceName"));
...
//        }
     ...
     * @return void
     */
    private void platformTask(String robotCode, String taskId, TCruiseResult tCruiseResult){
        try {
            synchronized (LOCK_FLAG){
                ...
                log.info("从redis获取的taskId为{}的总检测点数是==={}, 异常点数是==={}, 正常点数是==={}", taskId, totalNum, abnormalNum, normalNum);
    ...
    private void reanalysisResult(Long instanceId, Map<String, String> tCruiseTaskResultMap, String taskId, String inspectionCode){
        ...
        // 配置了算法
        if(details.getAnalyseType() != null || IS_AI_AlGORITHM.equals(details.getIsAi())) {
            ...
            if(IS_AI_AlGORITHM.equals(tStdDevicemete.getIsAi())){
